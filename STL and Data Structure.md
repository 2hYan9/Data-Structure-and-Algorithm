...
>二叉查找的思想不但可以用于二叉查找树中，也可以用某种特殊排列的矩阵的来实现：  
>比如现在有一个n$\times$m的矩阵，其每一行是从左往右递增序列，每一列也都是从上往下递增序列。现给定一个元素，判断这个元素是否是在这个矩阵中。  
>最容易想到的方法就是遍历这个矩阵，然后进行判断，这种方法的时间复杂度为`O(nm)`。  
>有另一种更快的方法，其时间复杂度为`O(n + m)`，该方法是，从矩阵的右上角开始，那么这个矩阵可以近似的当成二叉查找树来处理，如果当前元素比待查找的元素小，那么列数就减一，如果当前元素比待查找的元素大，那么行数就减一，直到找到待查找的元素。
